#This file has my most optimized version to get the lowest dev error rate
import pandas as pd
import numpy as np
...
X = df.drop(columns=["id", "target",'race','sex']).copy()
y = df["target"].copy()
...
('num', num_processor, ['hours']),
('cat', cat_processor, ['edu','marriage','occupation','age','sector'])
])
...
if "k" not in df.columns or len(df["k"]) != len(k_values):
    df = pd.DataFrame({"k": k_values})
df["smart_scaled_no_demo"] = dev_errors
df.to_csv(filename, index=False)
print(f"Saved results for smart_scaled_no_demo to {filename}")
...
#order columns
final_columns = [
    "id", "age", "edu", "marriage", "occupation",
      "hours", "target",'sector'
]
